...
interface ReimburseCandidate {
  id: number; postedAt: string; description: string; merchantNormalized: string | null; amount: string; categoryName: string | null; categoryColor: string | null;
}

function ReimbursePicker({ repayment, busy, onPick, onClose }: {
  repayment: Transaction; busy: boolean; onPick: (expenseId: number | null) => void; onClose: () => void;
}) {
  const [candidates, setCandidates] = useState<ReimburseCandidate[]>([]);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#00000088", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }} onClick={onClose}>
      <div style={{ background: "var(--bg-2)", border: "1px solid var(--border-2)", borderRadius: 10, padding: 24, width: 480, maxWidth: "90vw" }} onClick={(e) => e.stopPropagation()}>
        ...
        <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search expenses…"
        ...
        <div style={{ maxHeight: 320, overflowY: "auto", display: "flex", flexDirection: "column", gap: 2 }}>
          {loading ? (
            ...
            <button key={c.id} onClick={() => onPick(c.id)} disabled={busy}
              ...
              <span style={{ fontSize: 11, color: "var(--text-muted)", fontFamily: "var(--font-ibm-mono)", whiteSpace: "nowrap" }}>
                {formatTxDate(c.postedAt)}
              </span>
              ...
              <span style={{ fontFamily: "var(--font-ibm-mono)", fontSize: 13, color: "var(--expense)", whiteSpace: "nowrap" }}>
                {formatCurrency(parseFloat(c.amount), "MYR")}
              </span>
            </button>
          ))}
        </div>
